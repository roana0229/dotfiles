...
plugins=(git atom bower coffee node rbenv vagarnt)
...
function tmux_automatically_attach_session()
{
    if is_screen_or_tmux_running; then
        ...
        if is_tmux_runnning; then
            echo "${fg_bold[red]}Run tmux.${reset_color}"
        elif is_screen_running; then
            echo "${fg_bold[red]}This is on screen.${reset_color}"
        fi
    else
